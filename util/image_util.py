...
class Augmentor(object):
    ...
    def random_rotate(self, imgs, labels):
        ...
        def rotate(pair):
            ...
            transform = tf.image.angles_to_projective_transforms(angle, h, w)

            img = tf.image.transform(img, transform, interpolation='BILINEAR')
            img = tf.image.crop_to_bounding_box(img, height // 4, width // 4, height, width)

            label = tf.image.transform(label, transform, interpolation='BILINEAR')
